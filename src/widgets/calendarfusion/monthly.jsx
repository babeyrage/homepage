...
import { FUSION_COLORS, FUSION_MONO, MonoLabel } from "components/widgets/fusion/primitives";
...
export function Day({ weekNumber, weekday, events, colorVariants, showDate, setShowDate, currentDate }) {
  ...
  const isToday = cellDate.startOf("day").ts === currentDate.startOf("day").ts;
  const isSelected = cellDate.startOf("day").ts === showDate.startOf("day").ts;

  const dayStyles = (displayDate) => {
    let style = "h-9 tabular-nums ";
    ...
    style += isSelected ? "text-black-500 bg-black/10 dark:bg-white/10 rounded-md " : "";

    if (isToday) {
      style += "font-bold rounded-md ";
    } else {
  ...
  return (
    <button
      ...
      style={{
        width: "14%",
        fontFamily: FUSION_MONO,
        backgroundColor: isToday ? `${FUSION_COLORS.infra}26` : undefined,
        color: isToday ? FUSION_COLORS.infra : undefined,
      }}
          ...
          filteredEvents
            ...
            .map((event) => (
              <span
                ...
                className={classNames("inline-flex h-1 w-1 m-0.5 rounded-full", colorVariants[event.color] ?? "gray")}
              />
            ))}
      </span>
    </button>
        ...
        <div className="flex justify-between flex-wrap">
          {dayNames.map((name) => (
            <span key={name} className={classNames(cellStyle)} style={{ width: "14%" }}>
              <MonoLabel className="text-theme-500 dark:text-theme-400/70">{name}</MonoLabel>
            </span>
          ))}
        </div>
