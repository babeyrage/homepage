...
vi.mock("../calendar/agenda", () => ({
  default: ({ showDate }) => <div data-testid="calendar-agenda" data-show={showDate?.toISODate?.() || ""} />,
}));

import Component from "./component";

describe("widgets/calendarfusion/component", () => {
  it("renders monthly view by default", async () => {
    renderWithProviders(<Component service={{ widget: { type: "calendarfusion", integrations: [] } }} />, {
  ...
  it("renders agenda view when configured", async () => {
    renderWithProviders(
      <Component service={{ widget: { type: "calendarfusion", view: "agenda", integrations: [] } }} />,
      { settings: { hideErrors: false } },
    );
